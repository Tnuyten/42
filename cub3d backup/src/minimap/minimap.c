...
void	place_tile(t_mini *mini, t_uint x, t_uint y, t_map map)
{
	...
	x_start = x * mini->tile_width;// + mini->w_margin + mini->w_offset;
	y_start = y * mini->tile_height;// + mini->h_margin + mini->h_offset;
	yy = y_start;
	while (yy < y_start + mini->tile_height && yy < mini->height)
	{
		xx = x_start;
		while (xx < x_start + mini->tile_width && xx < mini->width)
		{
			if (map.map[y][x] == '1')
...
void	make_minimap(t_game *game)
{
	...
	mini->width = 750;
	mini->height = HEIGHT;
	mini->minimap = mlx_new_image(game->mlx, mini->width, mini->height);
	prepare(game);
	y = 0;
...
static void	prepare(t_game *game)
{
	...
	minimap->tile_width = minimap->width / map.width;
	minimap->tile_height = minimap->height / map.height;
	...
	minimap->h_margin = (minimap->height % map.height) / 2;
	minimap->w_margin = (minimap->width % map.width) / 2;
	if(map.width > map.height)
	{
		minimap->h_offset = (minimap->height - map.height * minimap->tile_height) / 2;
		minimap->w_offset = 0;
	}
	else
	{
		minimap->h_offset = 0;
		minimap->w_offset = (minimap->width - map.width * minimap->tile_width) / 2;
	}
